...
do
  ...
  do
    ...
    CUDA_VISIBLE_DEVICES=0,1 python src/main.py  \
        --train-data="./data/csvs/tunnel/integrated_laion_bddtunnel_cons1k_neg10t.csv" \
        --ct-data-location="./data/datasets/bdd100k"     \
        --base-model="./data/base_models/checkpoint_scene.pt"   \
        ...
        --task='scene' --ct-sampler --ct-num-cls-per-batch=4  --target-class=1 --lora --num-class=10 \
        --eval-datasets=BDD100k,BDD100k_Val,BDD100k_Test  --save=./checkpoints/    \
        --exp_name=bdd100k_tunnel/sogpnl_subsetD1k_E40B600lr6beta${beta}tau005t005
  done    
done 
