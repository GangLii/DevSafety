...
class CLIPEncoder(torch.nn.Module):
    ...
    def forward(self, images, text=None,cls_ids=None, raw_fea=None, return_raw = False):
        assert self.model is not None
        # if text==None:
        #     return self.model.encode_image(images)
        # else:
        return self.model(images, text, cls_ids, raw_fea, return_raw)
...
class ClassificationHead(torch.nn.Linear):
    def __init__(self, normalize, weights, biases=None, shape=[512, 6]): ##orginial flyp shape=[512,1000]
        if weights is not None:
            output_size, input_size = weights.shape #[1000,512] #[6,512]
    ...
    def forward(self, inputs):
        if self.normalize:
            inputs = inputs / inputs.norm(dim=-1, keepdim=True) ##L2 normalization
